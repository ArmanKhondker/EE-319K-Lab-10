...
LOOP1
	...
	AND R2,R2,#0X10;
	CMP R2,#0;
	BNE LOOP1;      CHECK FOR THE SSIO TO BE READY
	
	LDR R1,=DC;
	MOV R2,#0;
;	LDR R2,[R1]
;	AND R2,R2,#0XBF;
	STR R2,[R1];  MAKE THE THING GET COMMANDS
	
	LDR R1,=SSI0_DR_R;
	STR R0,[R1];      WRITING THE COMMAND 
...
LOOP2
	...
	AND R2,R2,#0X10;
	CMP R2,#0;
	BNE LOOP2;      CHECK FOR THE SSIO TO BE READY
...
LOOP3
	...
	AND R2,R2,#0X2;
	CMP R2,#0;
	BEQ LOOP3;      CHECK FOR THE TNF TO BE READY
	
	LDR R1,=DC;
	MOV R2,#0X40;
	STR R2,[R1];  MAKE THE THING GET DATA
	
	LDR R1,=SSI0_DR_R;
	STR R0,[R1];      WRITING THE DATA 
